...
-   **全功能围栏操作**：
    ...
    -   显示模式：**图标** 或 **列表**。
-   **随桌面壁纸自适应**：可选“随桌面自适应”——按围栏所在区域采样桌面壁纸平均色作为半透明背景，换壁纸自动刷新；栏底(标题栏)颜色一并同源压暗，保持可分辨；条目与标题字体颜色还会依据背景明暗自动在**黑/白**间切换，保证任何壁纸下都清晰可读。
...
`WallpaperTint` 采样壁纸区域平均色 + `SystemEvents.UserPreferenceChanged` 刷新
