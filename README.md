...
配置窗口按“预设主题 → 背景样式 → 标题栏样式 → 条目样式”组织；围栏右键「预设主题」/「自定义」均进入配置窗口（分别聚焦对应区块）。「预设主题」区可在「自适应/浅色/深色」间一键切换，背景/栏底/字色随之统一；手动改任意自定义配色会自动退出自适应（自定义色不被后续壁纸重算覆盖）。菜单中点选「⋯」按钮或右键标题栏可快速切换显示模式与溢出模式。
