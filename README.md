...
-   **全功能围栏操作**：
    -   标题栏拖动移动；双击标题栏重命名。
    -   四边/四角原生缩放（缩放光标跟随）。
    -   条目多选（Ctrl / Shift 点选整组）、整组拖放；可把条目拖到**其他软件**（仅复制，不移动源文件，避免引用死链）。
    -   点击围栏内外任意非选中区自动取消选择（全局低级鼠标钩子）。
    -   溢出模式：**滚动** 或 **省略**（超出容量折叠为“还有 N 项”）。
    -   显示模式：**图标** 或 **列表**。
-   **层叠模式（窗口层级）**：可在右键菜单或配置窗切换三种模式，即时生效、随配置持久化：
    -   **置底**（默认）：把窗口 **owner** 设为桌面 `SHELLDLL_DefView` → Win+D 不隐藏、沉在普通窗口之下（等同此前默认行为）。
    -   **置顶**：解除桌面 owner，用 `HWND_TOPMOST` 成为**始终置于所有普通窗口之上**的窗口；代价是按 Win+D 会像普通置顶窗一样被隐藏（Windows 置顶窗标准行为）。
    -   **窗口**：独立顶层窗口，随普通窗口 z 序、可点击聚焦、Win+D 会最小化、可 Alt+Tab（不入任务栏）。
    -   ⚠️ 取舍：**“Win+D 免疫（owner=桌面）”与“总在最前（置顶）”在 Windows 下互斥**——被拥有的窗口必须始终位于 owner 之上，而桌面 owner 位于 z 序最底层，二者只能取其一；“置顶”按“总在最前”实现。
-   **同步模式（文件夹活镜像）**：
    -   **手动**（默认）：自由增删，等同此前行为。
    -   **监视**：在弹出的配置窗里指定**文件夹地址**（含「浏览」按钮）+**后缀筛选**（如 `txt;*.png;lnk`，留空=全部），确定后围栏自动成为该文件夹的“活镜像”——文件**新增 / 删除 / 重命名**实时同步进围栏；双击打开仍指向真实文件/快捷方式；切回手动即停止监视并保留当前条目转为自由管理。重启后若原为监视模式会自动接管。
...
`ApplyLayerMode()` 按 `Fence.LayerMode` 切换：置底=`SetWindowLongPtr(GWL_HWNDPARENT, SHELLDLL_DefView)` 设 owner + `SendToBack` 沉底；置顶=清除桌面 owner + `SetWindowPos(HWND_TOPMOST)`（注意 owner 与 TOPMOST 互斥，挂桌面 owner 会令窗口被钉在桌面层、永远浮不到普通窗口之上）；窗口=清除 owner/Topmost/NOACTIVATE，成为独立顶层窗口。`OnSourceInitialized` 调用、`Closed` 释放。

同步模式

`ApplySyncMode()` + `FileSystemWatcher`（`NotifyFilter=FileName|DirectoryName`，后缀筛选靠 `MatchesFilter` 手动匹配）；Created/Deleted/Renamed/Error 回调均在 `Dispatcher.Invoke` 回 UI 线程增删/改路径；Error 后重建 watcher（缓冲溢出恢复）。进入监视先 `ResyncFromFolder()` 以文件夹当前内容全量重建。外部拖入在监视模式被忽略以防孤儿项。
