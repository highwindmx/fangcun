...
配置窗口按“背景样式 → 标题栏样式 → 条目样式”组织；围栏右键「主题」菜单可在「自适应/浅色/深色/自定义」间一键切换，背景/栏底/字色随之统一，菜单中点选「⋯」按钮或右键标题栏可快速切换显示模式与溢出模式。
...
`WallpaperTint` 采样壁纸区域明暗 → 生成中性玻璃色（RGB 与壁纸色相无关），`SystemEvents.UserPreferenceChanged` 刷新
